...
def generate_admin_items_sql():
    ...
    for tier_name, items in admin_data.items():
        ...
        for item_name, item_data in items.items():
            ...
            sql_lines.append("    name, slot_id, basedamage, basedefense, tier, subtier,")
            sql_lines.append("    depth_new, quality_new")
            ...
            sql_lines.append(f"    99,  -- tier: Admin Tier")
            sql_lines.append(f"    0,  -- subtier: Admin")
    ...
    sql_lines.append("    AVG(basedamage) as avg_damage,")
    sql_lines.append("    AVG(basedefense) as avg_defense,")
    sql_lines.append("    MAX(basedamage) as max_damage,")
    sql_lines.append("    MAX(basedefense) as max_defense")
    ...
    sql_lines.append("SELECT id, name, slot_id, basedamage, basedefense, quality_new")
    sql_lines.append("FROM items")
    sql_lines.append("WHERE depth_new = 99 AND quality_new = 'ADMIN'")
