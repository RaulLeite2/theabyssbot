...
class Sanctuary(commands.Cog):
    ...
    # =========================
    # HELPER: NOTIFICAÇÃO DE SANCTUARY
    # =========================
    
    async def check_sanctuary_in_zone(self, user_id: int, zone_id: int, previous_zone_id: int = None):
        """
        Verifica se há Sanctuary da guilda/aliança do player na zona atual
        ...
        # Procura Sanctuary da guilda ou da aliança nesta zona
        if alliance:
        ...
        if sanctuary:
            embed = discord.Embed(
                title="🏰 Sanctuary Detectado!",
                description=f"Esta zona possui o Sanctuary **{sanctuary['name']}**!",
                color=discord.Color.blue()
            )
            
            if sanctuary.get('alliance_name'):
                embed.add_field(
                    name="🤝 Pertence à",
                    value=f"Aliança **{sanctuary['alliance_name']}**",
                    inline=True
                )
            elif sanctuary.get('guild_name'):
                embed.add_field(
                    name="⚔️ Pertence à",
                    value=f"Guilda **{sanctuary['guild_name']}**",
                    inline=True
                )
            
            embed.add_field(
                name="⚡ Energia",
                value=f"{sanctuary['energy']}%",
                inline=True
            )
            embed.add_field(
                name="💡 Dica",
                value="Use `/sanc entrar` para acessar!",
                inline=False
            )
    ...
    async def durability_degradation_loop(self):
        """Loop que verifica Sanctuaries sem energia e degrada durabilidade."""
        import asyncio
        while not self.bot.is_closed():
            try:
                # A cada 30 minutos, verifica Sanctuaries sem energia
                ...
                for sanc in sanctuaries_without_energy:
                    current_durability = sanc["durability"] if sanc.get("durability") is not None else 100
                    ...
                    if new_durability <= 0:
                        ...
                        try:
                            ...
                            print(f"🏚️ Sanctuary '{sanc['name']}' (ID: {sanc['id']}) foi destruído por falta de energia.")
                        except Exception as e:
                            print(f"Erro ao deletar sanctuary {sanc['id']}: {e}")
                    else:
    ...
    async def clone_zone_for_sanctuary(self, base_zone_id: int, guild_id: int):
        """
        ...
        async with self.bot.db.acquire() as conn:
            async with conn.transaction():
                ...
                if is_zahuv_zone:
                    ...
                    # Atualiza a zona existente para ser permanente e sanctuary
                    await conn.execute(
                        """
                        UPDATE zone 
                        SET is_hideout = TRUE, 
    ...
    async def recharge_sanctuary(self, guild_id: int):
        """Recarrega energia do sanctuary."""
        try:
            sanctuary = await self.get_sanctuary(guild_id)
            if not sanctuary:
                return None, "❌ Sanctuary não encontrado."

            now = datetime.utcnow()
            last = sanctuary["last_recharge"]

            if last and now - last < timedelta(hours=1):
                restante = timedelta(hours=1) - (now - last)
                minutos = int(restante.total_seconds() // 60)
                return False, f"⏳ O Sanctuary ainda está recarregando.\nVolta em **{minutos} min**."
            ...
            return True, "⚡ **Sanctuary recarregado!** Energia no talo."
        except Exception as e:
            return None, f"❌ Erro ao recarregar sanctuary: {str(e)}"
    ...
    async def validate_guild_leader_and_sanctuary(self, user_id: int):
        ...
        sanctuary = await self.get_sanctuary(guild_id)
        if not sanctuary:
            return guild_id, None, "❌ Nenhum Sanctuary encontrado para sua guilda."

        return guild_id, sanctuary, None

    # ======================================================
    # 🏰 GRUPO /sanc E /ho (ALIAS)
    # ======================================================
    sanc = app_commands.Group(
        name="sanc",
        description="Comandos do Sanctuary (base de guilda)"
    )

    ho = app_commands.Group(
        name="ho",
        description="Comandos do Sanctuary (base de guilda) - Alias para /sanc"
    )

    # ========================
    # /sanc create e /ho create
    # ========================
    @sanc.command(name="create", description="Cria o Sanctuary da guilda (máx 7 por guilda)")
    async def sanc_create(self, interaction: discord.Interaction):
        import json
        ...
        if sanctuary_count >= 7:
            return await interaction.response.send_message(
                "⚠️ Sua guilda já possui 7 Sanctuaries (limite máximo).",
                ephemeral=True
            )
        ...
        # Se for zona de Zahuv (special_initial), verifica limite de 5 sanctuaries nessa zona
        if base_zone["nome"].startswith(("Ai'", "Et'", "Al'", "Jo'", "Ka'", "Lu'", "Xe'", "Ty'")):
            ...
            if existing_sanctuaries >= 5:
                return await interaction.response.send_message(
                    "⚠️ Esta zona de Zahuv já possui 5 Sanctuaries clonados (limite máximo).",
                    ephemeral=True
                )
            ...
            if existing_sanctuary:
                return await interaction.response.send_message(
                    "⚠️ Esta zona já possui um Sanctuary instalado!",
                    ephemeral=True
                )

        try:
            zone_id = await self.clone_zone_for_sanctuary(base_zone_id, guild_id)
        except Exception:
            return await interaction.response.send_message(
                "❌ Falha ao clonar a zona do Sanctuary.",
                ephemeral=True
            )
        ...
        if not fictional_zone_name:
            ...
            fictional_zone_name = zone_info["nome"] if zone_info else "Sanctuary"
        ...
        try:
            ...
            return await interaction.response.send_message(
                "❌ Falha ao criar o Sanctuary. Zona clonada foi removida.",
                ephemeral=True
            )

        await interaction.response.send_message(
            f"🏰 **Sanctuary criado!** A base da guilda **{guild_name}** agora existe na zona **{fictional_zone_name}** ({sanctuary_count + 1}/7)."
        )

    @ho.command(name="create", description="Cria o Sanctuary da guilda (máx 7 por guilda)")
    async def ho_create(self, interaction: discord.Interaction):
        await self.sanc_create(interaction)

    # ========================
    # /sanc info e /ho info
    # ========================
    @sanc.command(name="info", description="Mostra informações do Sanctuary")
    async def sanc_info(self, interaction: discord.Interaction):
        """Mostra informações do Sanctuary"""
        guild_id, sanctuary, error_msg = await self.validate_guild_leader_and_sanctuary(interaction.user.id)
        if error_msg:
            return await interaction.response.send_message(error_msg, ephemeral=True)

        embed = discord.Embed(
            title=f"🏰 Sanctuary: {sanctuary['name']}",
            color=discord.Color.dark_gold()
        )
        embed.add_field(name="🔼 Nível", value=sanctuary["level"], inline=False)
        embed.add_field(
            name="⚡ Energia",
            value=f"{sanctuary['energy']} / {sanctuary['max_energy']}",
            inline=False
        )
        
        # Mostra durabilidade se existir
        if sanctuary.get("durability") is not None:
            durability_bar = "█" * (sanctuary["durability"] // 10) + "░" * (10 - sanctuary["durability"] // 10)
            durability_status = "🟢 Ótimo" if sanctuary["durability"] > 70 else "🟡 Desgastado" if sanctuary["durability"] > 30 else "🔴 Crítico"
            embed.add_field(
                name="🛡️ Durabilidade",
                value=f"{durability_bar}\n{sanctuary['durability']}/100 | {durability_status}",
                inline=False
            )
            if sanctuary["energy"] == 0:
                embed.add_field(
                    name="⚠️ Aviso",
                    value="**Sem energia!** O Sanctuary está perdendo durabilidade. Recarregue-o urgentemente!",
                    inline=False
                )
        
        await interaction.response.send_message(embed=embed)

    @ho.command(name="info", description="Mostra informações do Sanctuary")
    async def ho_info(self, interaction: discord.Interaction):
        await self.sanc_info(interaction)

    # ========================
    # /sanc recharge e /ho recharge
    # ========================
    @sanc.command(name="recharge", description="Recarrega o Sanctuary")
    async def sanc_recharge(self, interaction: discord.Interaction):
        guild_id, sanctuary, error_msg = await self.validate_guild_leader_and_sanctuary(interaction.user.id)
        if error_msg:
            return await interaction.response.send_message(error_msg, ephemeral=True)

        success, msg = await self.recharge_sanctuary(guild_id)
        await interaction.response.send_message(msg)

    @ho.command(name="recharge", description="Recarrega o Sanctuary")
    async def ho_recharge(self, interaction: discord.Interaction):
        await self.sanc_recharge(interaction)

    # ========================
    # /sanc upgrade e /ho upgrade
    # ========================
    @sanc.command(name="upgrade", description="Evolui o Sanctuary")
    async def sanc_upgrade(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"🚀 **Sanctuary evoluído!**\n"
            f"Nível: **{sanctuary['level']} → {new_level}**\n"
            f"Energia Máxima: **{new_max_energy}**"
        )

    @ho.command(name="upgrade", description="Evolui o Sanctuary")
    async def ho_upgrade(self, interaction: discord.Interaction):
        await self.sanc_upgrade(interaction)

    # ========================
    # /sanc zone e /ho zone
    # ========================
    @sanc.command(name="zone", description="Mostra a zona do Sanctuary")
    async def sanc_zone(self, interaction: discord.Interaction):
        guild_id, sanctuary, error_msg = await self.validate_guild_leader_and_sanctuary(interaction.user.id)
        if error_msg:
            return await interaction.response.send_message(error_msg, ephemeral=True)
        ...
        if not zone:
            return await interaction.response.send_message(
                "❌ Zona do Sanctuary não encontrada.",
                ephemeral=True
            )

        # Monta embed
        embed = discord.Embed(
            title=f"🗺️ Zona do Sanctuary: {zone['nome']}",
        ...
        embed.add_field(name="Sanctuary Permanente?", value="✅" if zone["permanent"] else "❌")
        embed.add_field(name="É Sanctuary?", value="✅" if zone["is_hideout"] else "❌")

        await interaction.response.send_message(embed=embed)

    @ho.command(name="zone", description="Mostra a zona do Sanctuary")
    async def ho_zone(self, interaction: discord.Interaction):
        await self.sanc_zone(interaction)

    # ========================
    # /sanc list e /ho list
    # ========================
    @sanc.command(name="list", description="Lista todos os sanctuaries da sua guilda")
    async def sanc_list(self, interaction: discord.Interaction):
        guild = await self.bot.db.fetchrow(
            "SELECT id, name FROM guilds WHERE leader_id = $1",
            interaction.user.id
        )
        ...
        # Lista todos os sanctuaries da guilda
        ...
        if not sanctuaries:
            return await interaction.response.send_message(
                "📦 Sua guilda não possui sanctuaries ainda.\n\nUse `/sanc create` para criar um!",
                ephemeral=True
            )
        
        embed = discord.Embed(
            title=f"🏰 Sanctuaries de {guild['name']}",
            description=f"Total: **{len(sanctuaries)}/7** sanctuaries",
            color=discord.Color.dark_gold()
        )
        
        for sanc in sanctuaries:
            # Verifica se a zona ainda existe
            if sanc["zone_exists"] is None:
                status = "❌ **ZONA ÓRFÃ** (zona não existe mais)"
                zone_info = f"ID: `{sanc['zone_id']}` (deletada)"
            else:
                status = "✅ Ativo"
                zone_info = f"**{sanc['zone_name']}** (ID: `{sanc['zone_id']}`)"
            
            durability_info = ""
            if sanc.get("durability") is not None:
                durability_emoji = "🟢" if sanc["durability"] > 70 else "🟡" if sanc["durability"] > 30 else "🔴"
                durability_info = f" | {durability_emoji} Durabilidade: {sanc['durability']}/100"
            
            embed.add_field(
                name=f"#{sanc['id']} - {sanc['name']}",
                value=(
                    f"{status}\n"
                    f"🗺️ Zona: {zone_info}\n"
                    f"⚡ Energia: {sanc['energy']}/{sanc['max_energy']} | 🔼 Nível: {sanc['level']}{durability_info}"
                ),
                inline=False
            )
        
        embed.set_footer(text="Use /sanc cleanup para remover sanctuaries órfãos")
        await interaction.response.send_message(embed=embed)

    @ho.command(name="list", description="Lista todos os sanctuaries da sua guilda")
    async def ho_list(self, interaction: discord.Interaction):
        await self.sanc_list(interaction)

    # ========================
    # /sanc cleanup e /ho cleanup
    # ========================
    @sanc.command(name="cleanup", description="Remove sanctuaries com zonas que não existem mais")
    async def sanc_cleanup(self, interaction: discord.Interaction):
        guild = await self.bot.db.fetchrow(
            "SELECT id, name FROM guilds WHERE leader_id = $1",
            interaction.user.id
        )
        ...
        # Encontra sanctuaries órfãos (zona não existe mais)
        ...
        if not orphaned:
            return await interaction.response.send_message(
                "✅ Todos os sanctuaries da sua guilda estão OK!\n\nNenhuma zona órfã encontrada.",
                ephemeral=True
            )
        
        # Remove os sanctuaries órfãos
        deleted_count = 0
        deleted_names = []
        for sanc in orphaned:
            ...
            deleted_count += 1
            deleted_names.append(f"• **{sanc['name']}** (zona ID: `{sanc['zone_id']}`)")
        
        embed = discord.Embed(
            title="🧹 Cleanup Concluído!",
            description=f"Removidos **{deleted_count}** sanctuary(es) órfão(s):",
            color=discord.Color.green()
        )
        embed.add_field(
            name="Sanctuaries Removidos",
            value="\n".join(deleted_names),
            inline=False
        )
        embed.set_footer(text="Você pode criar novos sanctuaries com /sanc create")
        
        await interaction.response.send_message(embed=embed)

    @ho.command(name="cleanup", description="Remove sanctuaries com zonas que não existem mais")
    async def ho_cleanup(self, interaction: discord.Interaction):
        await self.sanc_cleanup(interaction)

    # ========================
    # /sanc delete e /ho delete
    # ========================
    @sanc.command(name="delete", description="Deleta um sanctuary específico pelo ID")
    @app_commands.describe(sanctuary_id="ID do sanctuary para deletar")
    async def sanc_delete(self, interaction: discord.Interaction, sanctuary_id: int):
        guild = await self.bot.db.fetchrow(
            "SELECT id, name FROM guilds WHERE leader_id = $1",
            interaction.user.id
        )
        ...
        # Verifica se o sanctuary existe e pertence à guilda
        sanctuary = await self.bot.db.fetchrow(
            "SELECT id, name, zone_id FROM hideouts WHERE id = $1 AND guild_id = $2",
            sanctuary_id, guild_id
        )
        
        if not sanctuary:
            return await interaction.response.send_message(
                f"❌ Sanctuary ID `{sanctuary_id}` não encontrado ou não pertence à sua guilda.",
                ephemeral=True
            )
        ...
        embed = discord.Embed(
            title="🗑️ Sanctuary Deletado!",
            description=f"**{sanctuary['name']}** (ID: `{sanctuary_id}`) foi removido.",
            color=discord.Color.red()
        )
        if zone_deleted:
            embed.add_field(
                name="Zona",
                value=f"✅ Zona `{sanctuary['zone_id']}` também foi deletada",
                inline=False
            )
        else:
            embed.add_field(
                name="Zona",
                value=f"⚠️ Zona `{sanctuary['zone_id']}` não foi encontrada (já estava deletada)",
                inline=False
            )
        
        await interaction.response.send_message(embed=embed)

    @ho.command(name="delete", description="Deleta um sanctuary específico pelo ID")
    @app_commands.describe(hideout_id="ID do sanctuary para deletar")
    ...
    # =========================
    # ENTRADA/SAÍDA DO SANCTUARY
    # =========================
    
    @sanc.command(name="entrar", description="Entra no Sanctuary da sua guilda/aliança na zona atual")
    async def sanc_entrar(self, interaction: discord.Interaction):
        # Verifica se o usuário está em uma guilda
        guild_member = await self.bot.db.fetchrow(
            "SELECT guild_id FROM guild_members WHERE user_id = $1",
            interaction.user.id
        ...
        if not guild_member:
            return await interaction.response.send_message(
                "❌ Você precisa estar em uma guilda para entrar em um Sanctuary!",
                ephemeral=True
            )
        ...
        if not sanctuary:
            return await interaction.response.send_message(
                "❌ Não há Sanctuary da sua guilda ou aliança nesta zona!",
                ephemeral=True
            )
        ...
        embed = discord.Embed(
            title=f"🏠 Bem-vindo ao {sanctuary['name']}!",
            description="Você entrou no Sanctuary da sua guilda/aliança.",
            color=discord.Color.green()
        )
        embed.add_field(name="⚡ Energia", value=f"{sanctuary['energy']}%", inline=True)
        embed.add_field(name="💪 Seu Power Score", value=f"`{power_score}`", inline=True)
        
        facilities = []
        if sanctuary['has_crafting_station']:
            facilities.append("🔨 Estação de Crafting")
        if sanctuary['has_dungeon_portal']:
        ...
        embed.set_footer(text="Use /sanc sair para retornar à zona anterior")
        
        await interaction.response.send_message(embed=embed)
    
    @ho.command(name="entrar", description="Entra no Sanctuary da sua guilda/aliança na zona atual")
    async def ho_entrar(self, interaction: discord.Interaction):
        await self.sanc_entrar(interaction)
    ...
    @ho.command(name="sair", description="Sai do Sanctuary e retorna à zona anterior")
    async def ho_sair(self, interaction: discord.Interaction):
        await self.sanc_sair(interaction)

    # =========================
    # CRAFTING SYSTEM
    # =========================
    
    @sanc.command(name="craft", description="Crafta um item especial no Sanctuary")
    @app_commands.describe(recipe_id="ID da receita para craftar")
    async def sanc_craft(self, interaction: discord.Interaction, recipe_id: int):
        # Verifica se está no sanctuary
        user = await self.bot.db.fetchrow(
        ...
        if not user or not user['in_hideout_id']:
            return await interaction.response.send_message(
                "❌ Você precisa estar dentro de um Sanctuary para craftar!",
                ephemeral=True
            )
        ...
        if not sanctuary['has_crafting_station']:
            return await interaction.response.send_message(
                "❌ Este Sanctuary não possui uma Estação de Crafting!",
                ephemeral=True
            )
        ...
        if recipe['min_hideout_level'] > sanctuary['level']:
            return await interaction.response.send_message(
                f"❌ Este Sanctuary precisa estar no nível {recipe['min_hideout_level']} para esta receita!",
                ephemeral=True
            )
    ...
    @ho.command(name="craft", description="Crafta um item especial no Sanctuary")
    @app_commands.describe(recipe_id="ID da receita para craftar")
    async def ho_craft(self, interaction: discord.Interaction, recipe_id: int):
        await self.sanc_craft(interaction, recipe_id)
    
    @sanc.command(name="recipes", description="Lista receitas de crafting disponíveis")
    async def sanc_recipes(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="📜 Receitas de Crafting do Sanctuary",
            description="Use `/sanc craft <recipe_id>` para craftar",
    ...
    async def ho_recipes(self, interaction: discord.Interaction):
        await self.sanc_recipes(interaction)

    # =========================
    # SANCTUARY DUNGEON SYSTEM
    # =========================
    
    @sanc.command(name="dungeon", description="Inicia a Dungeon especial do Sanctuary (requer 5 pessoas e 1500 power)")
    async def sanc_dungeon(self, interaction: discord.Interaction):
        # Verifica se está no sanctuary
        user = await self.bot.db.fetchrow(
            "SELECT in_hideout_id FROM users WHERE discord_id = $1",
            interaction.user.id
        ...
        if not user or not user['in_hideout_id']:
            return await interaction.response.send_message(
                "❌ Você precisa estar dentro de um Sanctuary!",
                ephemeral=True
            )
        
        sanctuary_id = user['in_hideout_id']
        
        # Verifica se o sanctuary tem portal de dungeon
        sanctuary = await self.bot.db.fetchrow(
            """
        ...
        if not sanctuary['has_dungeon_portal']:
            return await interaction.response.send_message(
                "❌ Este Sanctuary não possui Portal de Dungeon!",
                ephemeral=True
            )
        
        # Verifica cooldown
        if sanctuary['dungeon_cooldown'] and sanctuary['dungeon_cooldown'] > datetime.now():
            cooldown_end = int(sanctuary['dungeon_cooldown'].timestamp())
            return await interaction.response.send_message(
                f"❌ A Dungeon está em cooldown! Disponível <t:{cooldown_end}:R>",
                ephemeral=True
            )
        
        # Verifica se o usuário tem party ativa
        ...
        # Verifica se todos os membros estão no sanctuary
        members_in_sanc = []
        members_not_in_sanc = []
        total_power = 0
        ...
        for member_id in party.members:
            ...
            if member_data and member_data['in_hideout_id'] == sanctuary_id:
                ...
                members_not_in_sanc.append(f"<@{member_id}>")
        
        if len(members_in_sanc) < 5:
            msg = f"❌ A Dungeon requer 5 pessoas no Sanctuary! ({len(members_in_sanc)}/5)"
            if members_not_in_sanc:
                msg += f"\n\nMembros fora do Sanctuary: {', '.join(members_not_in_sanc)}"
        ...
        # Cria a dungeon run
        difficulty_tier = sanctuary['level']  # Dificuldade baseada no nível do Sanctuary
        ...
        run_id = await self.bot.db.fetchval(
            ...
            RETURNING id
            """,
            sanctuary_id, interaction.user.id, total_power, difficulty_tier
        )
        ...
        embed = discord.Embed(
            title=f"🌀 Dungeon Iniciada: {sanctuary['name']}",
            description=f"A party entrou na dungeon especial!",
            color=discord.Color.dark_purple()
        )
        embed.add_field(name="👥 Membros", value=str(len(members_in_sanc)), inline=True)
        ...
        member_list = "\n".join([f"<@{mid}>: `{pwr}` power" for mid, pwr in members_in_sanc])
        ...
        # Simula batalha
        await self._simulate_dungeon_battle(interaction, run_id, members_in_sanc, difficulty_tier)

    @ho.command(name="dungeon", description="Inicia a Dungeon especial do Sanctuary (requer 5 pessoas e 1500 power)")
    async def ho_dungeon(self, interaction: discord.Interaction):
        await self.sanc_dungeon(interaction)
    ...
    @sanc.command(name="facility", description="Adiciona instalações ao Sanctuary (Crafting/Dungeon)")
    ...
    async def sanc_facility(self, interaction: discord.Interaction, upgrade_type: str):
        ...
        if not sanctuaries:
            return await interaction.response.send_message(
                "❌ Sua guilda não possui Sanctuaries!",
                ephemeral=True
            )
        
        # Para simplificar, pega o primeiro sanctuary
        sanctuary = sanctuaries[0]
        ...
        if upgrade_type == "crafting":
            if sanctuary['has_crafting_station']:
                return await interaction.response.send_message(
                    "❌ Este Sanctuary já possui Estação de Crafting!",
                    ephemeral=True
                )
            ...
            embed = discord.Embed(
                ...
                description=f"**{sanctuary['name']}** agora possui uma estação de crafting!",
                color=discord.Color.gold()
            )
            ...
            if sanctuary['has_dungeon_portal']:
                return await interaction.response.send_message(
                    "❌ Este Sanctuary já possui Portal da Dungeon!",
                    ephemeral=True
                )
            ...
            embed = discord.Embed(
                ...
                description=f"**{sanctuary['name']}** agora possui acesso à dungeon especial!",
                color=discord.Color.purple()
            )
    ...
    @ho.command(name="facility", description="Adiciona instalações ao Sanctuary (Crafting/Dungeon)")
    @app_commands.describe(
        upgrade_type="Tipo de instalação: crafting, dungeon"
    )
    @app_commands.choices(upgrade_type=[
    ...
    async def ho_facility(self, interaction: discord.Interaction, upgrade_type: str):
        await self.sanc_facility(interaction, upgrade_type)

async def setup(bot: commands.Bot):
    await bot.add_cog(Sanctuary(bot))
